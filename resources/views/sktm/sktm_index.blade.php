    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                <a style="float:right;" href="{{  url('sktm/tambah') }}" class="btn btn-primary"><font size="2">TAMBAH DATA</font></a>
        ...
        <tbody>
           @foreach ($srt_ket_tdk_mampus as $sktm)
             <tr>
                    ...
                    <td><center>{{ $sktm->penduduk->nama }}</center></td>
                    <td><center>Rt. {{ $sktm->penduduk->kartukeluarga->rt }}</center></td>
                    <td><center>{{ nomorSuratSKTM($sktm->id) }}</center></td>
                    <td><center>{{ $sktm->tgl_keluar }}</center></td>
                    <td>
                        {{-- <center><a href="{{ url('sktm/edit/'.$sktm->id) }}"><i class="fa fa-pencil-square-o"></i>Info Surat</center> --}}
                        <a href="{{ url('sktm/cetak/'.$sktm->id) }}"><i class="fa fa-table"></i>Cetak Surat</center></a>
                            <button type="button" class="btn btn-sm btn-white text-danger border-danger" 
                            data-toggle="modal" 
                            data-target="#exampleModalCenter" 
                            data-id="{{$sktm->id}}" 
                            data-nama="{{$sktm->penduduk->nama}}">
...
<div class="modal fade exampleModalCenter" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      document.getElementById('formHapus').action='/hapusSKTM/'+id;
      })
    });
  </script>
@endsection
